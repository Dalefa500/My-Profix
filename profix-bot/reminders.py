...
же Telegram-бота, что шлёт заявки, напоминание за день до оплаты (у
отдельного платежа можно задать своё в "days_before").
...
SEND_FROM_HOUR = 10
DEFAULT_DAYS = [1]  # за день до оплаты — так решил владелец
...
def main(argv: list[str]) -> int:
    ...
    if "--test" in argv:
        sent = send(env, "✅ Проверка: напоминания об оплатах PROFIX работают.\n"
                         "Они будут приходить сюда за день до оплаты, в 10:00.")
