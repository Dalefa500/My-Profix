...
лишние порты, включит ежедневные резервные копии и покажет логины
с паролями учредителей.
...
**Если на сервере уже живёт другой сайт**, к строке `listen 80;` добавьте
`default_server`:
...
Запрос по голому IP-адресу не совпадает ни с одним `server_name`, и nginx
отдаёт его первому блоку из всех, что слушают 80-й порт, — а первым идёт
тот, чьё имя файла раньше по алфавиту. Пометка `default_server` закрепляет
безымянные запросы за нашим блоком. Соседние сайты от этого не страдают:
совпадение по имени всегда важнее. Двух блоков с `default_server` быть не
может — `nginx -t` такое не пропустит.

После `systemctl reload nginx` дайте секунду: старые рабочие процессы
дослуживают начатые запросы по прежним настройкам, и проверка сразу после
перезагрузки может показать прежний ответ.
